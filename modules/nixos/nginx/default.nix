...
let
  ...
  vhosts = concatStringsSep "\n" (
    mapAttrsToList (
      ...
      let
        ...
        hostListen = if vhost.forceSSL then filter (x: x.mode != null) defaultListen else defaultListen;
        ...
        listenString =
          ...
          + optionalString (mode == "ssl" && vhost.http2 && oldHTTP2) "http2 "
          + optionalString (mode != null) "${mode} "
          + optionalString vhost.default "default_server "
          + optionalString (vhost.reuseport && !(hasPrefix "unix:" addr)) "reuseport "
          + optionalString proxyProtocol "proxy_protocol "
          + optionalString (extraParameters != [ ]) (concatStringsSep " " extraParameters)
          + ";";

        redirectListen = filter (x: x.mode == null) defaultListen;
      ...
      ''
        ...
        server {
          ...
          ${optionalString (hasSSL && vhost.http2 && !oldHTTP2) ''
            http2 on;
          ''}
          ${optionalString (hasSSL && any (listen: listen.mode or null == "quic") hostListen) ''
  ...
  mkCertOwnershipAssertion = import "${modulesPath}/security/acme/mk-cert-ownership-assertion.nix" lib;
          ...
          listOf (submodule {
            options = {
              ...
              mode = mkOption {
                ...
                description = "Enable SSL or QUIC";
                default = null;
              };
